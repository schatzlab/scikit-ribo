...
class processAln(object):
    ...
    '''
    def __init__(self, bam=None, mapq=20, output=None, minRL=10, maxRL=35, startCodons=None, cds=None, posRanges=None,
                 RelE=None, nts=None):
        self.bam = bam
        self.mapq = mapq
        self.output = output
...
if __name__ == '__main__':
    ...
    if (args.i != None and args.p != None and args.o != None):
        ...
        sys.stderr.write("[execute]\tKeep reads with length [" + str(minRL) + ","+ str(maxRL) + "] and mapq >= " +
                         str(mapq) + "\n")
        aln = processAln(inBam, mapq, output, minRL, maxRL, start, cds, posIdx, RelE, nt)
